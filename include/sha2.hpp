...
namespace sha2 {
    ...
    public:
        ...
        constexpr void update(uint8_t const* data, size_t size) noexcept {
            buffer_total_ += size;
        ...
        constexpr void digest(uint8_t* hash) const noexcept {
    ...
    inline constexpr int SHA256_Update(SHA256_CTX *c, uint8_t const* data, size_t len) {
        c->update(data, len);
        return 1;
    }

    inline constexpr int SHA256_Final(uint8_t *md, SHA256_CTX *c) {
        c->finish();
        c->digest(md);
        return 1;
    }

    inline constexpr uint8_t* SHA256(uint8_t const* d, size_t n, uint8_t* md) noexcept {
        auto ctx = SHA256_CTX{};
        ctx.init();
        ctx.update(d, n);
        ctx.finish();
    ...
    inline constexpr int SHA512_Update(SHA512_CTX *c, uint8_t const* data, size_t len) {
        c->update(data, len);
        return 1;
    }

    inline constexpr int SHA512_Final(uint8_t *md, SHA512_CTX *c) {
        c->finish();
        c->digest(md);
        return 1;
    }

    inline constexpr uint8_t* SHA512(uint8_t const* d, size_t n, uint8_t* md) noexcept {
        auto ctx = SHA512_CTX{};
        ctx.init();
        ctx.update(d, n);
        ctx.finish();
